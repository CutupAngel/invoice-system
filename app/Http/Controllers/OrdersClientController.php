...
class OrdersClientController extends Controller
{
    ...
    public function postDeleteOptionData(Request $request)
    {
        ...
        switch ($request->get('type'))
        {
            case 'option':
                ...
                $deleted_name = $option->display_name;
                $option->delete();
                break;
            case 'value':
                ...
                $deleted_name = $value->display_name;
                $value->delete();
                break;
        }

        return redirect()->back()->with('status', $deleted_name . ' deleted successfully.');
    ...
    public function getGroup($id = 'new')
    {
        if ($id === 'new') {
            ...
            try {
                ...
                $group->permissionCheck($this->getUserId());
    ...
    public function getPackage($groupId, $packageId)
    {
        try {
            ...
            if($group->user_id != Auth::user()->id && Auth::user()->id != '1') $group->permissionCheck($this->getUserId());
            $group->url = urldecode($group->url);
        } catch (\Exception $e) {
            return redirect('/orders')->withErrors(['The selected group does not exist.']);
        }
        ...
        $ids = $package->link->pluck('option_id');
        ...
        $package_settings = PackageSetting::where('package_id', $package->id)->get();
    ...
    public function savePackage(Request $request, $groupId, $packageId = 'new')
    {
        ...
        $package->trial = $request->trial;
        $package->is_featured = $request->input('featured') == 'Y';
        $package->exclude_from_api = $request->input('exclude') == 'Y';
        $package->is_outofstock = $request->input('outofstock') == 'Y';
    ...
    {
        // $packageImage = Package_Image::findOrFail($request->id);
        Package_Image::where('id', $request->id)->delete();
        // $packageImage->delete(); 
        
        // return redirect('/orders')->with(['success' => 'Image deleted']);
    ...
    {
        if($request->data == $integration)
        {
          return Integrations::get($integration, 'getIntegrationServerForm', [$request]);
        }
        else
        {
    ...
    public function delete($groupId, $packageId = -1)
    {
        if ($packageId === -1) {
            ...
            if($delete->user_id != Auth::user()->id && Auth::user()->id != '1') $delete->permissionCheck($this->getUserId());
            $delete->delete();
        } else {
            ...
            if($delete->user_id != Auth::user()->id && Auth::user()->id != '1') $delete->group->permissionCheck($this->getUserId());
            $delete->delete();
        }

        return redirect('/orders')->with(['status' => $delete->name . ' was successfully deleted.']);
    }
    ...
    {
        $user = auth()->user();
        if ($user->id === 1) return $user->id;
        return $user->parentLink->parent_id;
    }
}
