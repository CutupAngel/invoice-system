...
class HelperController extends Controller
{
		...
		{
			if(Auth::user()->isCustomer()) return redirect(route('login'));
			...
			foreach ($userSearch as $user) {
				...
				$results["u{$user->id}"] = [
					'url' => $url,
					'text' => "{$type}, Name: {$user->name}, Username: {$user->username}, Email: {$user->email}"
				];
			}
			...
			foreach ($invoiceSearch as $invoice) {
				...
				$results["i{$invoice->id}"] = [
					...
					'text' => "Invoice: {$invoice->invoice_number}, Customer: {$invoice->name}, Total: {$invoice->total} &bull; Status: {$invoice->status}, Due: {$invoice->due_at}"
				];
			}
