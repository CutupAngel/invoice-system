...
@section('page.subtitle', 'Fixed Discounts')

@section('breadcrumbs')
	<a href="/marketing">Marketing</a>
	<li class="breadcrumb-item active">Fixed Discounts</li>
@stop
...
<button class="btn btn-success float-right" id="add-discount"><i class="fa fa-plus"></i> Add Fixed Discounts</button>
@stop
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title">@{{title}} Fixed Discounts</h4>
						<button type="button" class="close" data-dismiss="modal">×</button>
					</div>
					<div class="modal-body">
						<div class="form-group">
							<label for="value">Minimum Order:</label>
							<input type="number" class="form-control" id="value" name="value" value="@{{value}}" required>
						</div>
						<div class="form-group">
							<label for="discount">Discount Value:</label>
							<div class="input-group">
								<input type="number" class="form-control" id="discount" name="discount" min="0" max="100" value="@{{discount}}" required>
	...
	<script>
		(function($) {
			var table = $('#discounts').DataTable({
				'ajax': '/marketing/fixed-discounts',
				'columns': [
					{ 'data': 'value' },
					{ 'data': 'discount' },
					{ 'data': 'start' },
			...
			$('body').on('submit', '#discount-form', function() {
				$.ajax({
					url: '/marketing/fixed-discounts/' + $(this).find('[name="id"]').val(),
					type: 'PUT',
					data: $(this).serialize(),
				})
				.done(function(id) {
					if(isNaN(id))
          {
			...
			$('#discounts').on('click', '.edit', function() {
				...
				$.get('/marketing/fixed-discounts/' + id, function(data) {
			...
			$('#discounts').on('click', '.delete', function() {
				...
				if (confirm("Are you sure you want to delete this discount?")) {
					$.ajax({
						url: '/marketing/fixed-discounts/' + id,
						type: 'DELETE',
					})
					.done(function() {
						table.ajax.reload();
